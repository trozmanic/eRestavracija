...
$(document).ready(function () {
    ...
    $("#ZaposlenModal2").on("shown.bs.modal", function () {
        ...
        console.log("Modal se je pojavil")
        ...
        for (var i = 0; i < steviloVrstic; i++) {
            if (checkboxi[i].checked) {
                id = checkboxi[i].getAttribute("idzaposlenega");
            }
        }
        console.log(id);
        document.getElementById("uredi_urnik").setAttribute("href","/nadzorna_plosca/urnik/"+id);
    })
    preveriIme = false;
    preveriPosto = false;
...
uredi.addEventListener("click", function () {
    ...
    for (var i = 0; i < steviloVrstic; i++) {
        if (checkboxi[i].checked) {
            ...
            if (izbranaVloga != "Izberi vlogo:" && izbranaVloga != "") {
                posodobljenZaposlen.vloga = izbranaVloga;
            }

            axios.put("/api/uporabniki/", posodobljenZaposlen).then((odgovor) => {
            ...
            if (placa > 0) {
                ...
                axios.put('/api/zaposleni/', novaPlaca).then((odgovor) => {
                    console.log("Odgovor: " + odgovor.data);
                    document.getElementById("ImeForm2").value = "";
                    document.getElementById("PostaForm2").value = "";
                    document.getElementById("TelefonForm2").value = "";
